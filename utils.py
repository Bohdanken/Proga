...
def get_text_df(chosen_date):
    ...
    feature_names = cv.get_feature_names_out()
    divided_text['keywords'] = divided_text['data_lemmatized'].apply(lambda x: conver_doc_to_vector(x)) # (x, cv, tfidf)
    return divided_text
...
def write_mean_values():
    list_of_columns_needed = ["day_tempmax",
                            ...
                            "hour_severerisk"]
    output = dict()
    df_all_weather = pd.read_csv("data"+sep+"weather_alarms_regions"+sep+"all_weather_by_hour_v2.csv", sep=",")
    for i in df_all_weather.columns.values.tolist():
        if i in list_of_columns_needed:
            output[i] = df_all_weather[i].mean()
    pd.DataFrame.from_dict([output]).to_csv("data"+sep+"weather_alarms_regions"+sep+"all_weather_by_hour_v2_mean_values.csv", sep=',', encoding='utf-8')
